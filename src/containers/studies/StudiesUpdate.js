...
import '../../styles/studies/StudiesUpdate.css';

import axios from 'axios';
import $ from 'jquery';
window.$ = $;

const StudiesUpdate = ({ session, }) => {
  const UpdateF = () => {
    ...
    axios({
        url: '/api/study/'+pathnameId(current)+'/update',
        method: 'POST',
        data: {
          s_category: $('#studiesUpdate_categoryInput').val(),
          studycontent: $('#studiesUpdate_descriptionInput').val(),
          s_location: $('#studiesUpdate_locationInput').val(),
          maxrequirement: $('#studiesUpdate_max').val(),
          minrequirement: $('#studiesUpdate_min').val(),
    ...
    useEffect(() => {
      axios({
          ...
          $("#studiesUpdate_titleInput").val(data.studytitle);
          $("#studiesUpdate_descriptionInput").val(data.studycontent);
          $("#studiesUpdate_locationInput").val(data.s_location);
          $("#studiesUpdate_categoryInput").val(data.s_category);
          $("#studiesUpdate_max").val(data.maxrequirement);
          $("#studiesUpdate_min").val(data.minrequirement);
    ...
    <div id='body_main' style={{ background:'rgb(240, 240, 240)', }}>
      ...
      <div style={{ width: '60%', height: '850px', display: 'inline-block', borderRadius: '10px', background: 'white', }}>
        ...
        <div style={{ width: '98%', margin: '10px 15px', textAlign: 'center', }}>
          <button 
            style={{ margin: '10px 0px', color: 'white', borderRadius: '5px', backgroundColor: '#17173D', }} 
            onClick={() => UpdateF()}>
              글수정
            </button>
          </div>
